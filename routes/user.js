...
userRouter.post("/api/add-to-cart", keycloak.protect(),  async (req, res) => {
  try {
    const { id } = req.body;
...
userRouter.delete("/api/remove-from-cart/:id", keycloak.protect(), async (req, res) => {
  try {
    const { id } = req.params;
    const product = await Product.findById(id);
    let user = await User.findById(req.user);
...
// save user address
userRouter.post("/api/save-user-address", keycloak.protect(), async (req, res) => {
  try {
...
// order product
userRouter.post("/api/order", keycloak.protect(), async (req, res) => {
  try {
...
// get user's orders
userRouter.get("/api/orders/me", keycloak.protect(), async (req, res) => {
  try {
...
// get user's wallet
userRouter.get("/api/wallets/me", keycloak.protect(), async (req, res) => {
  try {
...
userRouter.get("/api/transaction/me", keycloak.protect(), async (req, res) => {
  try {
    let user = await User.findById(req.user);
...
userRouter.get("/api/transaction/exchange/me", keycloak.protect(), async (req, res) => {
  try {
    let user = await User.findById(req.user);
...
userRouter.get("/api/tasks/me", keycloak.protect(), async (req, res) => {
  try {
    let user = await User.findById(req.user);
...
userRouter.get("/api/tasks/inprogress/me", keycloak.protect(), async (req, res) => {
  try {
    let user = await User.findById(req.user);
...
userRouter.get("/api/tasks/done/me", keycloak.protect(), async (req, res) => {
  try {
    let user = await User.findById(req.user);
...
userRouter.put("/api/tasks/:id", keycloak.protect(),  async (req, res) => {
  if (!req.body) {
